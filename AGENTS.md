...
- Use the established AHD logo and identity from AHDGame/AHDClient. Do not substitute a new lettermark or generated logo. Keep the approved ethereal globe landing, and carry the canonical identity into launcher/account surfaces and platform app icons with source rights/provenance and offline assets recorded.
- Authentication must match AHDClient's observed account/session behavior. It is an active priority independent of the later full MP gameplay adapter. Inspect the reference before implementation; preserve the existing auth protocol, login/logout/restore/expiry behavior and offline SP boundary. The current AHDClient reference opens the live game in a persistent WebView: AHDGame owns authentication, remote content has no native privileges, and SP requires no account. Do not invent a Native token/account store. A local character profile is not an authenticated account. Do not introduce a separate identity backend or route users through another client app.
