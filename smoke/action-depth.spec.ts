...
test('domestic shares can be bought, partly sold and retained across relaunch', async ({ page }) => {
  ...
  const openMarket = async () => {
    ...
    await page.getByRole('menuitemradio', { name: 'Stock market', exact: true }).click();
...
test('an elected player proposes a tax rate and reopens the actual bill after relaunch', async ({ page }) => {
  ...
  await endTurn.click(); // The real saved sponsorship cooldown ends at turn 99.
  await expect(endTurn).toBeEnabled();
  const openBills = async () => {
    await page.getByRole('button', { name: 'Menu', exact: true }).click();
    await page.getByRole('menuitemradio', { name: 'Bills and proposals', exact: true }).click();
