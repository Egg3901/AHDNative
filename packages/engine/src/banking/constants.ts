...
export function effectiveDepositRatePercent(
  primeRate: number,
  depositOffset: number,
): number {
  return Math.max(MIN_DEPOSIT_RATE_PERCENT, primeRate + depositOffset);
}
export function effectiveLendingRatePercent(
  primeRate: number,
...
export function computeNpcDepositShare(
  ...
  const cbApy = Number.isFinite(centralBankSavingsApyPercent)
    ? centralBankSavingsApyPercent
    : 0;
...
export function equityCappedDepositCeiling(
  ...
  const capacity =
    typeof capacityCeiling === "number" && Number.isFinite(capacityCeiling)
      ...
      : 0;
  const equity =
    typeof bankEquity === "number" && Number.isFinite(bankEquity)
      ? bankEquity
      : 0;
  return Math.min(
    capacity,
    Math.max(0, equity) * NPC_DEPOSIT_MAX_EQUITY_LEVERAGE,
  );
}
...
export function computeInsurancePremium(
  ...
  const actual =
    typeof reserveRatioActual === "number" &&
    Number.isFinite(reserveRatioActual)
      ? Math.max(0, reserveRatioActual)
      : 0;
  const required =
    typeof reserveRatioRequired === "number" &&
    Number.isFinite(reserveRatioRequired)
...
export function sumInsuredPlayerDeposits(
...
export function computeReserveRatioActual(
  cashReserves: number,
...
export const CREDIT_BANDS: readonly CreditBand[] = [
  ...
  { id: "B", demandShare: 0.1, ratePremiumPp: 4.5, defaultRatePercent: 8.0 },
  {
    ...
    demandShare: 0.05,
    ratePremiumPp: 8.0,
    defaultRatePercent: 15.0,
  },
];
const BAND_BY_ID = new Map<CreditBandId, CreditBand>(
  CREDIT_BANDS.map((b) => [b.id, b]),
);
...
export function bandsForProfile(profileId: LendingProfileId): CreditBand[] {
  ...
  return CREDIT_BANDS.filter(
    (band) => CREDIT_BAND_IDS.indexOf(band.id) <= floorIndex,
  );
}

/** Source: creditBands.ts bandRatePercent (verbatim). */
export function bandRatePercent(
  band: CreditBand,
...
export function computeNpcLoanBookVolume(
  lendableDeposits: number,
  lendingRatePercent: number,
): number {
  const funding =
    Number.isFinite(lendableDeposits) && lendableDeposits > 0
      ? lendableDeposits
      : 0;
  const rate = Number.isFinite(lendingRatePercent) ? lendingRatePercent : 0;
  const volumeFactor = clamp(
    1 -
      (rate - NPC_LOAN_BOOK_RATE_REFERENCE_PERCENT) *
...
export function computeConfidence(input: ConfidenceInput): ConfidenceResult {
  ...
  const penalized =
    raw -
    CONFIDENCE_PANIC_PENALTY_PER_TURN *
      Math.min(panicTurns, CONFIDENCE_PANIC_TURNS_CAP) -
    stigma -
    (input.forcedLiquidation === true
