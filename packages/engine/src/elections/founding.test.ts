  ...
  function resolveAll(world: ReturnType<typeof createWorld>) {
    for (const rec of world.elections) {
      ...
      if (rec.candidates.length === 0) {
        rec.candidates = [{ id: "cand-a", name: "A", partyId: "US_DEM", isNPP: false, incumbent: false }];
      }
...
describe("founding convergence through the real turn loop (#223)", () => {
  function optedWorld(seed: string, races = 1) {
    const world = createWorld({ seed, playerName: "Player", countryId: "US", era: "1953" });
    const rng = rngFromSeed(seed);
    expect(stampFoundingMarker(world)).toBe(true);
    expect(runFoundingSweep(world, rng, races)).toBe(races);
    world.meta.rng = rng.state();
    return world;
  }

  it("createWorld opt-in stamps the marker and seats cycle-0 races", () => {
    const world = createWorld({
      seed: "founding-opt-in",
      playerName: "Player",
      countryId: "US",
      era: "1953",
    ...
    expect(world.elections.every((rec) => rec.cycle === 0)).toBe(true);
    expect(isFoundingActive(world.elections)).toBe(true);
  });

  it("resolves founding races, clears the marker, and resumes the calendar", () => {
    const world = optedWorld("founding-converge", 1);
  ...
  it("freezes the calendar date while the phase is active", () => {
    const world = optedWorld("founding-freeze", 3);
    const startDate = world.meta.date;
  ...
  it("survives a save/reload round trip mid-phase", () => {
    const world = optedWorld("founding-savereload", 2);
    advanceTurn(world);
