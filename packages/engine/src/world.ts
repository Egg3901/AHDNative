...
export function createWorld(options: NewWorldOptions): WorldState {
  ...
  // #223 founding opt-in: stamp the lifecycle marker, then run the dedicated
  // bounded founding sweep once (the solo analogue of the reference founding
  // sweep, driven by the turn loop's own registry in
  // src/lib/turn/foundingElections.ts). The normal election-timer planners
  // bail while the marker is active (see elections/founding.ts), so the
  // sweep calls pickNextCanonicalCycle directly. Candidate fill consumes
  // rng, so the captured rng state is refreshed to keep save/load
  // deterministic.
