...
function shellProps(
  ...
  return {
    ...
    loadRegions: async (query?: Parameters<typeof projectRegions>[1]) =>
      projectRegions(source, query),
...
describe("issue #84 nation context, wallet, and map links", () => {
  it("keeps the browsed nation across the World map round trip without touching save or turn", async () => {
    ...
    await user.selectOptions(
      await screen.findByRole("combobox", { name: "Nation view" }),
      "FR",
    );
    expect(
    ...
    await user.click(
      screen.getByRole("button", { name: "Open France in World map" }),
    );
    expect(
      await screen.findByRole("heading", { name: "World map" }),
    ).toBeInTheDocument();
    await user.click(
  ...
  it("exposes Profile, Actions, and Wallet in the drawer identity without corp/union rows", async () => {
    ...
    expect(
      await screen.findByRole("region", { name: "Portfolio" }),
    ).toBeInTheDocument();
  ...
  it("cross-links the explicit wallet/portfolio and banking destinations", async () => {
    ...
    expect(screen.getByRole("region", { name: "Portfolio" })).toHaveTextContent(
      "Wallet balances",
    );
    await user.click(screen.getByRole("button", { name: "Go to banking" }));
    expect(
      await screen.findByRole("region", { name: "Banking" }),
    ).toBeInTheDocument();
