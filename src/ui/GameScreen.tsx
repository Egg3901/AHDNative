...
const REGION_LABELS: Record<Exclude<RouteId, TabId>, string> = {
  ...
  presidentialDetails: "Presidential election",
  politicalMetrics: "Political metrics",
...
export function GameScreen({
  ...
  const [actionsCategory, setActionsCategory] =
    useState<ActionsCategoryFilter>("all");
  ...
  const [searchSnapshot, setSearchSnapshot] = useState<SearchPanelSnapshot>(
    EMPTY_SEARCH_SNAPSHOT,
  );
  const updateSearchSnapshot = useCallback(
    (next: SearchPanelSnapshot) => setSearchSnapshot(next),
    [],
  );
  ...
  const resourceButtonRefs = useRef<
    Partial<Record<ResourceId, HTMLButtonElement | null>>
  >({});

  const saveNotice =
    message === "Game saved." || message === "Saved game imported.";
  ...
  const clock: GameClock = {
    ...
    ...(typeof world.foundingOffset === "number"
      ? { foundingOffset: world.foundingOffset }
      : {}),
  };
  ...
  const holdingsCurrencies = [
    ...new Set(holdings.map((holding) => holding.currency)),
  ];
  const holdingsSummary =
    holdings.length > 0
      ? {
          ...
          total:
            holdingsCurrencies.length === 1
              ? formatFinanceMoney(
                  holdings.reduce(
                    (sum, holding) => sum + holding.shares * holding.price,
                    0,
                  ),
                  holdingsCurrencies[0]!,
                )
              : null,
        }
      : null;
  ...
  const openParty = (id: string) => {
    setDetailId(id);
    focusPage.current = true;
    setRoute("partyDetails");
  };
  const openElection = (id: string) => {
    setDetailId(id);
    focusPage.current = true;
    setRoute("electionDetails");
  };
  const openCampaign = (id: string) => {
    setDetailId(id);
    focusPage.current = true;
    setRoute("campaignDetails");
  };
  const openPolitician = (id: string) => {
    setDetailId(id);
    focusPage.current = true;
    setRoute("politicians");
  };
  // #69: the dedicated presidential race destination, opened from the Elections surface.
  const openPresidential = (id: string) => {
    setDetailId(id);
    focusPage.current = true;
    setRoute("presidentialDetails");
  };
  ...
  const openSearchResult = (result: SearchResult) => {
    ...
    const destinations: Record<SearchResult["kind"], RouteId> = {
      nation: "nations",
      party: "partyDetails",
      company: "markets",
      election: "electionDetails",
      bill: "legislationDetails",
  ...
  const electionPageCount = Math.max(
    1,
    Math.ceil(world.elections.length / ELECTIONS_PAGE_SIZE),
  );
  const safeElectionPage = Math.min(
    Math.max(0, electionPage),
    electionPageCount - 1,
  );
  const pagedElections = world.elections.slice(
    safeElectionPage * ELECTIONS_PAGE_SIZE,
    (safeElectionPage + 1) * ELECTIONS_PAGE_SIZE,
  );
  ...
  useEffect(() => {
    setElectionPage((p) =>
      Math.min(
        ...
        Math.max(
          0,
          Math.ceil(world.elections.length / ELECTIONS_PAGE_SIZE) - 1,
        ),
      ),
    );
  }, [world.elections.length]);

  const activeRaceId = world.elections.find(
    (e) => e.playerCandidate && e.status !== "resolved",
  )?.id;
  useEffect(() => {
    setElectionPage(0);
  }, [activeRaceId]);
  ...
  const presidentialRaceView = world.elections.find(
  ...
  return (
    ...
    >
      <div className="ahd-dual-body">
        {dual ? drawer : null}
        <main
          aria-hidden={(menuOpen && !dual) || undefined}
          inert={menuOpen && !dual}
          className="ahd-container ahd-main"
          data-pane={dual ? "content" : undefined}
        >
          <h1 className="ahd-sr-only">A House Divided · {pageTitle(route)}</h1>
          {busy && message ? (
            <div
              className="ahd-notice"
              role="status"
              aria-live="polite"
          ...
          {isTabRoute(route) ? (
            ...
            >
                  ...
                  <RouteHero
                    image={
                      world.player.mode === "hos"
                        ? executiveHero(world.countryId)
                        : "/static/heroes/actions.webp"
                    }
                    alt={
                      world.player.mode === "hos"
                        ? `${world.countryName} executive office`
                  ...
                  >
                    {world.player.mode === "hos" ? (
                      <div className="ahd-notice" role="note">
                        ...
                        >
                          ...
                          <strong>
                            Permanent Head of State ·{" "}
                            {world.player.currentOffice ?? "executive office"}
                          </strong>
                        </span>
                        ...
                        <div className="ahd-help">
                          Fiscal directions enact on the next turn.
                        </div>
                      </div>
                    ) : null}
                    <div
                    ...
                    >
                      <span>
                        ...
                        {world.player.partyName || "Independent"}
                      </span>
                      <span className="ahd-badge">
                        {world.player.actions} actions
                      </span>
                      <span className="ahd-badge">
                        {world.player.influence.toLocaleString(undefined, {
                          maximumFractionDigits: 1,
                        })}{" "}
                        influence
                      </span>
              ...
              {route === "parties" ? (
                <div className="ahd-stack">
                  <RouteHero
                    image="/static/heroes/parties.webp"
                    alt="Political party gathering"
                    eyebrow={world.countryName}
                    title="Political parties"
                  >
                    <button
                      className="ahd-btn ahd-btn-sm"
                      onClick={() => go("partyManagement")}
                    >
                      Start a party
                    </button>
                    <button
                    ...
                    >
                      {world.parties.length} parties in {world.countryName}
                    </p>
                    <p
                      className="ahd-help"
                      role="note"
                    ...
                    >
                      Switching parties or leaving your party withdraws your
                      candidacy.
                    </p>
                  </RouteHero>
                  {world.parties.length === 0 ? (
                    ...
                    <div className="ahd-grid ahd-grid-3">
                      {world.parties.map((p) => {
                        const membershipAction = p.isPlayerParty
                          ? (p.membership?.leave ?? leavePartyAction)
                          : (p.membership?.join ?? joinPartyAction);
                        const label = p.isPlayerParty
                          ? `Leave ${p.name}`
                          : `Join ${p.name}`;
                        const disabled = busy || !membershipAction?.available;
                        const hint = !membershipAction
                          ? "Unavailable"
                          : !membershipAction.available
                            ? (membershipAction.disabledReason ?? "Unavailable")
                            : membershipAction.cost > 0
                              ? `Cost ${membershipAction.cost} actions`
                              : "Free";
                        const handleMembership = () => {
                          if (disabled || !membershipAction) return;
                          if (p.isPlayerParty)
                            onAction(membershipAction.id, undefined);
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <div
                                style={{
                                  width: `${Math.min(100, p.members / 50)}%`,
                                  height: "100%",
                                  background: p.color,
                                }}
                              />
                            ...
                            {!membershipAction?.available &&
                            membershipAction?.disabledReason ? (
              ...
              {route === "legislature" ? (
                <div className="ahd-stack">
                  <button
                    className="ahd-btn"
                    onClick={() => go("legislationDetails")}
                  >
                    Browse bills and proposals
                  </button>
                  <LegislaturePanel
                    legislature={world.legislature}
                    busy={busy}
                    onAction={onAction}
                    clock={clock}
                  />
                ...
                <div className="ahd-stack">
                  <div className="ahd-card ahd-card-pad ahd-hero">
                    ...
                    <p
                      ...
                      style={{ fontSize: "0.76rem", marginTop: "0.25rem" }}
                    >
                      {world.elections.length} elections
                    </p>
                    {presidentialRaceView ? (
                      <button
                        ...
                        onClick={() =>
                          openPresidential(presidentialRaceView.id)
                        }
                        disabled={busy}
                      >
                        Presidential race
                    ...
                    {electionPageCount > 1 ? (
                      ...
                      >
                        <button
                          type="button"
                          className="ahd-btn ahd-btn-sm"
                          onClick={() => setElectionPage(safeElectionPage - 1)}
                          disabled={busy || safeElectionPage === 0}
                        ...
                        <span
                          ...
                          aria-live="polite"
                        >
                          Page {safeElectionPage + 1} of {electionPageCount}
                        </span>
                        <button
                          type="button"
                          className="ahd-btn ahd-btn-sm"
                          onClick={() => setElectionPage(safeElectionPage + 1)}
                          disabled={
                            busy || safeElectionPage >= electionPageCount - 1
                          }
                  ...
                  {world.elections.length === 0 ? (
                    ...
                    <div className="ahd-grid ahd-grid-2">
                      {pagedElections.map((e) => {
                        ...
                        const isWithdraw =
                          candidacy?.id === "withdrawCandidacy";
                        const candidacyLabel = isWithdraw
                          ? "Withdraw candidacy"
                          : "Run for office";
                        const candidacyDisabled = busy || !candidacy?.available;
                        const candidacyHint = !candidacy
                          ? "Unavailable"
                          : !candidacy.available
                            ? (candidacy.disabledReason ?? "Unavailable")
                            : candidacy.cost > 0
                              ? `Cost ${candidacy.cost} actions`
                              : "Free";
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <span className="ahd-pill">
                                {RACE_PHASE_LABELS[e.phase]}
                              </span>
                              {e.playerCandidate ? (
                                <span
                                  className="ahd-pill"
                            ...
                            <div
                              ...
                              style={{ fontSize: "0.74rem" }}
                            >
                              {e.status} · {formatGameDate(e.date, clock)}
                            </div>
                            <div
                              className="ahd-muted"
                              style={{ fontSize: "0.74rem" }}
                            >
                              Filing deadline:{" "}
                              {formatGameDate(e.filingDate, clock) || "Unknown"}
                            </div>
                            {e.countedVotes != null ? (
                              <div
                                className="ahd-muted"
                                style={{ fontSize: "0.74rem" }}
                                aria-label={`${e.title} counted tally`}
                              >
                                ...
                                {e.leaderName && e.leaderShare != null
                                  ...
                                  : ""}
                                {e.seatProjection
                                  ? ` · seats ${e.seatProjection.map((seat) => `${seat.name} ${seat.seats}`).join(", ")}`
                                  : ""}
                              </div>
                            ) : null}
                            ...
                            >
                              <button
                                type="button"
                                className="ahd-btn ahd-btn-primary ahd-btn-sm"
                                onClick={handleCandidacy}
                                disabled={candidacyDisabled}
                                aria-disabled={candidacyDisabled}
                                aria-label={candidacyLabel}
                              >
                                {candidacyLabel}
                              </button>
                              <span
                            ...
                            {e.electionType === "president" ? (
                              <button
                                type="button"
                                className="ahd-btn ahd-btn-ghost ahd-btn-sm"
                                onClick={() => openPresidential(e.id)}
                              >
                                View presidential race
                              </button>
                            ) : null}
                            {e.playerCandidate && e.status !== "resolved" ? (
                              <button
                                type="button"
                                className="ahd-btn ahd-btn-ghost ahd-btn-sm"
                                onClick={() => openCampaign(e.id)}
                              >
                                Manage campaign
                              </button>
                            ) : null}
                            {!candidacy?.available &&
                            candidacy?.disabledReason ? (
                              <p className="ahd-help" role="note">
                                {candidacy.disabledReason}
                              </p>
                            ) : null}
              ...
              {route === "news" ? (
                world.news.length === 0 ? (
                  ...
                  <NewsPanel
                    ...
                    storageKey={
                      newsStorageKey ??
                      `${world.era}:${world.countryId}:${world.player.name}`
                    }
                    onCountry={(id) => navigate("nations", id)}
                    onParty={openParty}
                    onElection={openElection}
                  />
                )
              ) : null}
            </section>
            ...
            >
              ...
              {(route === "nations" || route === "state") && (
                <DetailQuery
                  load={loadWorldOverview}
                  revision={world}
                  label="World details"
                >
                  {(overview) => (
              ...
              {route === "worldMap" && (
                <WorldMapRoute
                  loadOverview={loadWorldOverview}
                  loadRegions={loadRegions}
                  revision={world}
                  section={preferences.worldMapSection}
                  onSectionChange={(worldMapSection) =>
                    onPreferencesChange({ ...preferences, worldMapSection })
                  }
                  onNavigate={navigate}
                />
              )}
                      ...
                      <CabinetOfficePanel
                        ...
                        notice={
                          error
                            ...
                            : message
                              ? { kind: "ok", text: message }
                              : null
                        }
                        onIssue={onIssueCabinetOrder}
                      />
                    )}
                  </DetailQuery>
                )}
              ...
              {route === "partyManagement" && (
                <DetailQuery
                  load={loadPartyManagement}
                  revision={world}
              ...
              {route === "search" && (
                <SearchPanel
                  ...
                  onSnapshot={updateSearchSnapshot}
                />
              )}
              {route === "markets" && (
                <MarketsRoute
              ...
              {route === "legislationDetails" && (
                <LegislationRoute
                  initialId={detailId}
                  countryId={world.countryId}
                  load={loadLegislation}
                  revision={world}
                  busy={busy}
                  onAction={onAction}
                />
              ...
              {route === "worldSettings" && (
                <WorldSettingsPanel
                  ...
                  busy={busy}
                  onChange={onUpdateWorldFeatureFlags}
                />
              )}
              {route === "ask" && (
                <div className="ahd-ask-embed">
                  <AskPanel surface="main" onBeforeSignIn={onSave} />
                </div>
              )}
              {route === "profile" ? (
                <ProfileRoute
                  ...
                  viewerDisablesAutoplay={
                    preferences.disableAutoplayOnOtherProfiles
                  }
                  onNavigate={(next, id) => {
              ...
              {(route === "partyDetails" ||
                ...
                <button
                  ...
                  onClick={() =>
                    route === "campaignDetails"
                      ? setRoute("electionDetails")
                      : go(route === "partyDetails" ? "parties" : "elections")
                  }
                >
                  Back to{" "}
                  {route === "partyDetails"
              ...
              {(route === "presidentialDetails" ||
                route === "politicalMetrics") && (
                <button
                  className="ahd-btn ahd-btn-ghost ahd-btn-sm"
              ...
              {route === "electionDetails" && (
                <PoliticsRoute
                  ...
                  onOpenPolitician={openPolitician}
                  onOpenPresidential={openPresidential}
                  busy={busy}
                  onAction={onAction}
                  clock={clock}
                />
      ...
      <footer
        ...
        inert={menuOpen && !dual}
        ref={footerRef}
        className="ahd-footer"
        aria-label="Status and primary navigation"
      >
        <div className="ahd-container ahd-footer-inner">
          <div className="ahd-statusline">
            ...
            <span className="ahd-status-identity">
              ...
              <span className="ahd-muted ahd-status-identity-context">
                {world.player.partyName || "Independent"} · {world.countryName}
              </span>
            </span>
            <span className="ahd-mono">
              Turn {world.turn} ·{" "}
              {world.foundingActive === true ? (
                <span
                  className="ahd-founding-badge"
                  title="In-game date is frozen at the era start until the founding elections finish"
                >
                  Founding
                </span>
              ) : null}
              {world.foundingActive === true ? " " : null}
              {formatGameDate(world.date, clock)}
            </span>
            {saveNotice && !busy && !error ? (
              <span className="ahd-muted" role="status">
                {message}
              </span>
            ) : (
          ...
          {holdingsSummary ? (
            <div
              className="ahd-muted"
              aria-label="Corporation holdings"
              style={{
                fontSize: "0.72rem",
                display: "flex",
                gap: "0.4rem",
          ...
          <div
            className="ahd-status-resources"
            role="group"
          ...
          >
            ...
            {RESOURCES.map((r) => {
              const value =
                r.id === "ap"
                  ? formatCount(world.player.actions)
                  : r.id === "funds"
                    ? formatFinanceMoney(
                        world.player.funds,
                        world.finance.currency,
                    ...
                    : r.id === "cash"
                      ...
                      : r.id === "influence"
                        ? formatCount(world.player.influence)
              ...
              return (
                <button
                  ...
                  type="button"
                  ref={(el) => {
                    resourceButtonRefs.current[r.id] = el;
                  }}
                  ...
                  onClick={() => {
                    setPreviewOpen(false);
                    setOpenResource(open ? null : r.id);
                  }}
                >
                  <span className="ahd-status-btn-label">
                    {r.id === "influence"
                      ? "INF"
                      : r.id === "favorability"
                        ? "FAV"
                  ...
                  <span className="ahd-mono ahd-status-btn-value">
                    {r.id === "funds" || r.id === "cash"
                      ? formatCompactMoney(
                          r.id === "funds"
                            ? world.player.funds
                            : world.finance.cash,
        ...
        {previewOpen && !openResource ? (
          ...
          >
            <NotificationPreview
              ...
              onDelete={onDeleteNotification}
              onOpenInbox={() =>
                openNotificationTarget({ route: "notifications" })
              }
            />
          ...
          >
            <div className="ahd-resource-details">
              <div className="ahd-resource-facts">
                {openResource === "ap" ? (
                  <span>
                    Available action points: {formatCount(world.player.actions)}
                    .
                  </span>
                ) : null}
                {openResource === "funds" ? (
                ...
                {openResource === "cash" ? (
                  <span>
                    ...
                    . Savings:{" "}
                    {formatFinanceMoney(
                      world.finance.savings,
                      world.finance.currency,
                    )}
                    .
                  </span>
                ) : null}
                {openResource === "influence" ? (
                  <span>Influence: {formatCount(world.player.influence)}.</span>
                ) : null}
                {openResource === "favorability" ? (
                  <span>
                    Favorability: {formatCount(world.player.favorability)}.
                  </span>
                ) : null}
              </div>
              <ResourceBreakdown
                details={world.resources}
              ...
              <div className="ahd-resource-links">
                <button
                  ...
                  onClick={() => {
                    ...
                    setActionsCategory(
                      openResource === "funds" || openResource === "cash"
                        ? "fundraising"
                        : "influence",
                    );
                    go("actions");
