...
function makeMap(section: "nations" | "regions" = "nations") {
  ...
  const regions = projectRegions(world, {
    directoryPage: 0,
...
describe("WorldMapPanel", () => {
  it("lists the actual projected nations and regions with no plotted coordinates", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "World map" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `${overview.nations.length} nations · ${regions.directoryTotal} regions in ${regions.playerCountryName}`,
      ),
    ).toBeInTheDocument();
    // Every projected nation is selectable into the existing Nations route.
    expect(
      screen.getByRole("button", { name: "Open United States nation details" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Open France nation details" }),
    ).toBeInTheDocument();
    // Every projected region row is selectable into the existing Regions route.
    expect(
      screen.getByRole("button", {
        name: `Open ${regions.directory[0]!.name} region details`,
      }),
    ).toBeInTheDocument();
    ...
    expect(
      document.querySelector("[data-lat],[data-lon],[data-coordinates]"),
    ).toBeNull();
    expect(
      screen.getByText(/the save records no coordinates/),
    ).toBeInTheDocument();
  });
  ...
  it("opens nation and region details through the existing routes", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Open California region details" }),
    );
  ...
  it("searches the nation directory without touching the region directory", () => {
    ...
    fireEvent.change(
      screen.getByRole("searchbox", {
        name: "Search nations on the world map",
      }),
      { target: { value: "France" } },
    );
    expect(
      screen.queryByRole("button", {
        name: "Open United States nation details",
      }),
  ...
  it("persists only the supported section choice and reorders the sections", () => {
    ...
    expect(
      within(toggle).getByRole("button", {
        name: "Show nations section first",
      }),
    ).toHaveAttribute("aria-pressed", "true");
    fireEvent.click(
      within(toggle).getByRole("button", {
        name: "Show regions section first",
      }),
    );
  ...
  it("renders the regions section first when that is the stored preference", () => {
    ...
    expect(sections.indexOf("Regions")).toBeLessThan(
      sections.indexOf("Nations"),
    );
  });
  ...
  it("keeps Hall of Fame explicitly unavailable instead of fabricating a leaderboard", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /hall of fame|leaderboard/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("links only to nation and region details, never to fabricated election or profile rows", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /election|profile|leaderboard/i }),
    ).not.toBeInTheDocument();
  });
