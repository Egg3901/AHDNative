 ...
 *
 * Bounded contract on top of #359/#387: the ready screen offers a compact
 ...
 * state via the shared footer. Nothing embeds or navigates to another
 * client app: no credential forms, no token store, no webview.
 */
...
describe("MpModeScreen single-view drill-in at 320px", () => {
  ...
  it("uses wrap-safe CSS so the section nav never clips at 320px", () => {
    ...
    expect(css).toMatch(/\.ahd-mp-sections[^{]*\{[^}]*flex-wrap:\s*wrap/);
...
describe("MpModeScreen single-view sign-in at 390px", () => {
  it("states the single-view sign-in behavior and offers a safe return path", async () => {
    ...
    expect(screen.getByText(/briefly leaves this screen/)).toBeInTheDocument();
    expect(screen.queryByText(/AHDClient|full-site|full site/i)).toBeNull();
    expect(screen.queryByRole("iframe")).toBeNull();

    const navigation = within(screen.getByRole("navigation", { name: "Primary" }));
    expect(navigation.getByRole("button", { name: "Ask" })).toBeInTheDocument();
    expect(navigation.getByRole("button", { name: "Menu" })).toBeInTheDocument();
