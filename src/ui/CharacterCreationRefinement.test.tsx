...
describe("CharacterCreationScreen phone-first refinement (#242)", () => {
  ...
  it("summarises the home region choice with its position in the option list", async () => {
    const user = userEvent.setup();
    render(<CharacterCreationScreen {...props()} />);
    await reachStep(user, 3);
    expect(screen.getByTestId("creation-region-summary")).toHaveTextContent("New York: 1 of 2 home states.");
    await user.selectOptions(screen.getByRole("combobox"), "CA");
    expect(screen.getByTestId("creation-region-summary")).toHaveTextContent("California: 2 of 2 home states.");
  });
  ...
  it.each([320, 390])("keeps every refined control reachable at a %dpx viewport", async (width) => {
    ...
    expect(screen.getByTestId("creation-region-summary")).toBeInTheDocument();
