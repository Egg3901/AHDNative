...
describe("MobileNavigation", () => {
  ...
  it.each([
    ...
    "keeps the parent destination marked while viewing %s",
    (route, label) => {
      render(
        <BottomNav
          route={route}
      ...
      expect(screen.getByRole("button", { name: label })).toHaveAttribute(
        "aria-current",
        route === label.toLowerCase() ? "page" : "location",
      );
  ...
  it("mirrors the reference menu hierarchy: no Character group, Actions top-level, Nation/World sub-groups", () => {
    ...
    expect(MENU_GROUPS.map((g) => g.label)).toEqual([
      "Profile",
      "Actions",
      "Ask",
      "State",
    ...
    expect(nation.sections?.map((s) => s.label)).toEqual([
      "Politics",
      "Government",
      "Economy",
    ]);
    ...
    expect(nation.sections?.[1]!.items.map((i) => i.label)).toEqual([
      "Legislature",
      "Bills and proposals",
      "Policy",
      "Cabinet office",
    ]);
    expect(nation.sections?.[2]!.items.map((i) => i.label)).toEqual([
      "Economy",
      "National Budget",
      "National Metrics",
    ]);
    ...
    expect(world.sections?.map((s) => s.label)).toEqual([
      "Economy",
      "Diplomacy",
      "Other",
    ]);
    expect(world.sections?.[0]!.items.map((i) => i.id)).toEqual([
      "markets",
      "bonds",
      "banking",
    ]);
    expect(world.sections?.[1]!.items.map((i) => i.id)).toEqual([
      "nations",
      "worldMap",
    ]);
    expect(world.sections?.[2]!.items.map((i) => i.id)).toEqual([
      "news",
      "worldSettings",
    ]);
  });
  ...
  it("drawer renders the reference group and sub-section headings and keeps turn actions open", async () => {
    ...
    for (const label of ["Politics", "Government", "Economy"]) {
      expect(
        within(nation).getByRole("group", { name: label }),
      ).toBeInTheDocument();
    }
    const world = within(menu).getByRole("group", { name: "World" });
    await user.click(within(world).getByRole("button", { name: "World" }));
    expect(
      within(world).getByRole("group", { name: "Economy" }),
    ).toBeInTheDocument();
    expect(
      within(world).getByRole("button", { name: "Stock market" }),
    ).toBeInTheDocument();
    expect(
      within(world).getByRole("button", { name: "Bonds" }),
    ).toBeInTheDocument();
    expect(
      within(world).getByRole("button", { name: "Banking" }),
    ).toBeInTheDocument();
    // #352: the in-game World administration surface lives under World > Other.
    expect(
      within(world).getByRole("button", { name: "World settings" }),
    ).toBeInTheDocument();
    await user.click(
      within(world).getByRole("button", { name: "World settings" }),
    );
    ...
    expect(
      within(nation).getByRole("button", { name: "National Metrics" }),
    ).toBeInTheDocument();
    // #69: the reference Politics group also carries the presidential race page
    // and the political-metrics registry.
    expect(
      within(nation).getByRole("button", { name: "Presidential election" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("button", { name: "Political metrics" }),
    ).toBeInTheDocument();
    ...
    expect(onClose).not.toHaveBeenCalled();
    await user.click(
      screen.getByRole("button", { name: "Bills and proposals" }),
    );
  ...
  it("collapses deep groups into touch-friendly disclosure controls", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "National Budget" }),
  ...
  it("expands the active deep group when opened on its route (#366)", () => {
    ...
    expect(screen.getByRole("button", { name: "Nation" })).toHaveAttribute(
      "aria-expanded",
      "false",
    );
    expect(
    ...
    expect(screen.getByRole("button", { name: "Economy" })).toHaveAttribute(
      "aria-current",
      "page",
    );
  });
  ...
  it("resets deep groups to collapsed when reopened on a shallow route (#366)", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Stock market" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the unread badge on the Notifications entry (#366)", () => {
    ...
    expect(
      within(screen.getByRole("button", { name: "Notifications" })).queryByText(
        "3",
      ),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders a compact identity header with every fact truncated (#366)", () => {
    ...
    render(
      <GameDrawer
        ...
        route="profile"
        busy={false}
        playerName="Ada Lovelace the First of Her Very Long Name"
        playerParty="Labor"
        countryName="United States"
        turn={12}
        date="1953-12-01"
    ...
    const name = screen.getByText(
      "Ada Lovelace the First of Her Very Long Name",
    );
    ...
    expect(css).toMatch(
      /\.ahd-drawer-identity\s*\{[^}]*padding:\s*0\.35rem 0\.9rem 0\.5rem/,
    );
    expect(css).toMatch(
      /\.ahd-drawer-identity-name\s*\{[^}]*text-overflow:\s*ellipsis/,
    );
    expect(css).toMatch(
      /\.ahd-drawer-identity-meta\s*\{[^}]*white-space:\s*nowrap/,
    );
  });
  ...
  it("disclosures expose counts and controlled sections without renaming groups (#366)", async () => {
    ...
    expect(nation).toHaveAttribute(
      "aria-controls",
      "ahd-drawer-section-nation",
    );
    ...
    expect(css).toMatch(
      /\.ahd-drawer-group\s*\+\s*\.ahd-drawer-group\s*\{[^}]*border-top:/,
    );
  });
  ...
  it("keeps Ask/Actions pinned in a quick bar without removing destinations (#366)", async () => {
    ...
    const goActions = within(quick).getByRole("button", {
      name: "Go to Actions",
    });
    ...
    expect(css).toMatch(
      /\.ahd-drawer-quick\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/,
    );
    expect(css).toMatch(
      /\.ahd-drawer-quick\s*\{[^}]*env\(safe-area-inset-bottom\)/,
    );
  });
  ...
  it("holds phone safe-area, backdrop and focus behavior at 320/390px (#366)", async () => {
    ...
    expect(screen.getByRole("button", { name: "End turn" })).not.toBe(
      document.activeElement,
    );
    // Backdrop dismisses; closing returns focus to the Menu button.
    await user.click(
      document.querySelector(".ahd-drawer-backdrop") as HTMLElement,
    );
    ...
    expect(screen.getByRole("button", { name: "Menu" })).toBe(
      document.activeElement as HTMLElement,
    );
    // Phone sheet rules: left safe area, full-bleed backdrop, narrow-column
    // tightening at 320/390px, drawer width unchanged.
    expect(css).toMatch(
      /\.ahd-drawer\s*\{[^}]*padding-left:\s*max\(0,\s*env\(safe-area-inset-left\)\)/,
    );
    expect(css).toMatch(
      /\.ahd-drawer-backdrop\s*\{[^}]*position:\s*fixed;\s*inset:\s*0/,
    );
    expect(css).toMatch(
      /@media\s*\(max-width:\s*390px\)[\s\S]*?\.ahd-drawer-nav\s*\{[^}]*gap:\s*0\.2rem/,
    );
    expect(css).toMatch(
      /\.ahd-drawer\s*\{[^}]*width:\s*min\(19rem,\s*calc\(100vw - 3\.5rem\)\)/,
    );
  });
  ...
  it("exposes the reference avatar/profile identity flow and the wallet destination", () => {
    ...
    const diplomacy = groups
      .get("World")!
      .sections?.find((s) => s.label === "Diplomacy");
  ...
  it("docks the drawer as an in-flow navigation pane without modal behavior (#438)", async () => {
    ...
    expect(
      within(pane).getByRole("button", { name: "End turn" }),
    ).toBeInTheDocument();
