/**
 * Banking hero imagery (#386, slice of #143).
 *
...
describe("banking hero imagery", () => {
  ...
  it("falls back to the Actions artwork for unbundled and unknown countries", () => {
  ...
  it("uses truthful alts grounded in the bundled bytes and the reference hero config", () => {
    // US/UK repeat the reference `centralBank.heroAlt` (AHDGame
    // `src/lib/constants/countries.ts`) verbatim. JP carries no reference
    // alt, so the alt follows the upstream file identity recorded in the
    // hero-route manifest (File:Bank of Japan 2010.jpg,
    // "Bank of Japan, Chuo-ku Tokyo Japan").
...
describe("banking hero 320px/390px/desktop contract (#386)", () => {
  // jsdom performs no layout, so these cases assert the shipped shrink/wrap
  // styles on the hero balance rows, the wrapping transfer controls, and the
  // shared crop breakpoints, matching the MetricRowContracts convention.
  it("keeps hero balances and the holder label inside the row at 320px", () => {
    render(
      <FinancePanel
        finance={makeFinance({
          cash: 1234567890.5,
          savings: 987654321.25,
          savingsHolder: "A very long central savings institution holder name that must wrap",
        })}
        section="banking"
        countryId="US"
        busy={false}
    ...
    const holder = screen.getByText(/very long central savings/) as HTMLElement;
    expect(holder.style.overflowWrap).toBe("anywhere");
  });

  it("wraps the deposit/withdraw controls instead of clipping at 320px", () => {
    render(
      <FinancePanel finance={makeFinance()} section="banking" countryId="JP" busy={false} onAction={vi.fn()} />,
    );
    const deposit = screen.getByRole("button", { name: /deposit/i });
    const controls = deposit.closest("div")?.parentElement as HTMLElement;
