...
const loadPolitics = async () => ({
  ...
  referendumRequest: {
    ...
    action: {
      ...
      cost: 0,
      available: false,
      disabledReason: "Referendums are UK-only.",
    },
  },
  politicians: [],
});
  ...
  return {
    ...
    nation: {
      ...
      budget: {
        ...
        links: [
          { label: "Policy", route: "policy" as const },
          { label: "Legislature", route: "legislature" as const },
        ],
        taxRates: [],
            ...
            metrics: [
              {
                ...
                history: [],
                modifiers: [],
                links: [{ label: "Economy", route: "economy" as const }],
              },
            ],
          },
        ],
...
async function navigate(
  ...
  if (!destination) {
    ...
    if (collapsedGroup) {
      await user.click(
        menu.getByRole("button", { name: collapsedGroup.label }),
      );
...
describe("GameScreen", () => {
  it("opens on the player profile, matching the existing game entry flow", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /save game/i }),
    ).not.toBeInTheDocument();
    ...
    expect(
      await within(profile).findByRole("heading", { name: "Ada" }),
    ).toBeInTheDocument();
  ...
  it("keeps single-pane phone navigation when no hinge is reported (#438)", () => {
    ...
    expect(
      screen.queryByRole("complementary", { name: "Game navigation" }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("docks navigation beside content when separated segments are reported (#438)", () => {
    (
      window as unknown as { getViewportSegments: () => unknown }
    ).getViewportSegments = () => [
    ...
    try {
      ...
      expect(
        screen.getByRole("complementary", { name: "Game navigation" }),
      ).toBeInTheDocument();
      expect(
        screen.queryByRole("dialog", { name: "Game menu" }),
      ).not.toBeInTheDocument();
      expect(
        screen.getByRole("navigation", { name: "Primary" }),
      ).toBeInTheDocument();
      ...
      delete (window as unknown as { getViewportSegments?: unknown })
        .getViewportSegments;
    }
  });
  ...
  it("keeps the own-profile context across linked destinations and return", async () => {
    ...
    const profile = screen.getByRole("region", { name: "Profile" });
    await user.click(
      await within(profile).findByRole("button", { name: "Representative" }),
    );
    expect(
      screen.getByRole("region", { name: "Legislature" }),
    ).toBeInTheDocument();
    await navigate(user, "Profile");
    expect(
      await within(screen.getByRole("region", { name: "Profile" })).findByRole(
        "heading",
        { name: "Ada" },
      ),
    ).toBeInTheDocument();
  ...
  it("filters the offline wire and restores the selected read article with related links", async () => {
    ...
    await user.selectOptions(
      screen.getByRole("combobox", { name: "News category" }),
      "Election",
    );
    await user.selectOptions(
      screen.getByRole("combobox", { name: "News country" }),
      "US",
    );
    await user.selectOptions(
      screen.getByRole("combobox", { name: "News date" }),
      "1953-02-01",
    );
    ...
    await user.click(
      screen.getByRole("button", { name: "Read General election called" }),
    );
    const article = screen.getByRole("article", {
      name: "General election called",
    });
    expect(article).toHaveTextContent("Voters will choose a new House.");
    expect(
      within(article).getByRole("button", { name: "View United States" }),
    ).toBeInTheDocument();
    expect(
      within(article).getByRole("button", { name: "View Labor" }),
    ).toBeInTheDocument();
    expect(
      within(article).getByRole("button", { name: "View General Election" }),
    ).toBeInTheDocument();
    expect(
      within(article).getByRole("region", { name: "Event context" }),
    ).toHaveTextContent("Election call");
    ...
    await navigate(user, "News");
    expect(
      screen.getByRole("article", { name: "General election called" }),
    ).toBeInTheDocument();
    ...
    await navigate(user, "News");
    expect(
      screen.queryByRole("article", { name: "General election called" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Read General election called" }),
    ).toBeInTheDocument();
  ...
  it("lands article related links on real country, party, and election destinations", async () => {
    ...
    const props = {
      ...preferencesProps,
      newsStorageKey: "article-links-slot",
      loadProfile: async () => profileFor(world),
      loadPolitics,
      search,
      loadBondMarket,
    ...
    await user.click(
      screen.getByRole("button", { name: "View United States" }),
    );
    expect(
      await screen.findByRole("region", { name: "Nations" }),
    ).toBeInTheDocument();
  });
  ...
  it("preserves the viewed nation and player state across a Nations browse round trip", async () => {
    ...
    expect(
      within(screen.getByRole("group", { name: "Nation context" })).getByRole(
        "note",
      ),
  ...
  it("reflects busy disabling actions and shows message and error", async () => {
    ...
    expect(
      within(screen.getByRole("dialog", { name: "Game menu" })).getByRole(
        "alert",
      ),
  ...
  it("performs action with params and respects disabledReason", async () => {
      ...
      actions: [
        {
          ...
          cost: 1,
          available: false,
          disabledReason: "Need more influence",
        },
        {
          id: "fundraise",
          name: "Fundraise",
    ...
    await navigate(user, "Actions");
    expect(screen.getAllByText("Need more influence").length).toBeGreaterThan(
      0,
    );
    const takeButtons = screen.getAllByRole("button", { name: /take action/i });
    const available = takeButtons.find((b) =>
      b.textContent?.includes("Fundraise"),
    );
    expect(available).toBeTruthy();
    const amountInput = screen.getByLabelText(
      /amount for fundraise/i,
    ) as HTMLInputElement;
  ...
  it("validates amount before invocation and shows error for invalid", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /take action: fundraise/i }),
    );
    expect(onAction).not.toHaveBeenCalled();
  ...
  it("action labels distinguish which action", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /take action: advertise/i }),
    ).toBeInTheDocument();
  });
  ...
  it("handles party and region required actions via actual props", async () => {
    ...
    expect(onAction).toHaveBeenCalledWith(
      "endorse",
      expect.objectContaining({ partyId: "p2" }),
    );
  ...
  it("does not invoke when party or region selection missing", async () => {
    ...
    await navigate(user, "Actions");
    await user.click(
      screen.getByRole("button", { name: /take action: join party/i }),
    );
    expect(onAction).not.toHaveBeenCalled();
  });
  ...
  it("keeps national figures on Economy, reached through the game menu", async () => {
    ...
    expect(screen.getByRole("region", { name: "Economy" })).toHaveTextContent(
      "GDP",
    );
  ...
  it("opens World settings from the game menu and submits the complete flag map", async () => {
    const user = userEvent.setup();
    const eventsLabel = WORLD_FEATURE_FLAG_DEFINITIONS.find(
      (definition) => definition.key === "events",
    )!.label;
    const world = makeWorld({
      featureFlags: { ...DEFAULT_WORLD_FEATURE_FLAGS, events: false },
    });
    ...
    render(
      <GameScreen
        ...
        onSave={vi.fn()}
        onExit={vi.fn()}
        onUpdateWorldFeatureFlags={onUpdateWorldFeatureFlags}
        onAction={vi.fn()}
      />,
    );
    expect(
      screen.queryByRole("region", { name: "World settings" }),
    ).not.toBeInTheDocument();
    await navigate(user, "World settings");
    expect(
      screen.getByRole("region", { name: "World settings" }),
    ).toBeInTheDocument();
    expect(screen.getAllByRole("checkbox")).toHaveLength(
      WORLD_FEATURE_FLAG_DEFINITIONS.length,
    );
    expect(
      screen.getByRole("checkbox", { name: eventsLabel }),
    ).not.toBeChecked();
    await user.click(screen.getByRole("checkbox", { name: eventsLabel }));
    expect(onUpdateWorldFeatureFlags).toHaveBeenCalledTimes(1);
    expect(onUpdateWorldFeatureFlags).toHaveBeenCalledWith({
      ...DEFAULT_WORLD_FEATURE_FLAGS,
      events: true,
    });
  });
  ...
  it("opens the national metrics registry and follows its linked destinations", async () => {
    ...
    expect(
      within(metrics).getByText("National metrics registry"),
    ).toBeInTheDocument();
  ...
  it("election card shows filing deadline, badge, candidates and runs for office", async () => {
    ...
    const world = makeWorld({
      elections: [
        makeElection({ playerCandidate: true, candidateNames: ["Ada", "Bob"] }),
      ],
    });
    render(
      <GameScreen
    ...
    expect(
      within(card).getByText(/November, Week 1, 1953/),
    ).toBeInTheDocument();
    ...
    expect(onAction).toHaveBeenCalledWith("declareCandidacy", {
      electionId: "e1",
    });
  });
  ...
  it("shows counted tally, margin and seat chips when the tally has data", async () => {
    ...
    expect(
      within(card).getByText(
        /10,000 counted · Ada 60\.0% \(\+20\.0pt\) · seats Ada 6/,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("shows a corporation holdings strip in the footer when the player holds shares", async () => {
    ...
    expect(
      within(strip).getByText(/Holdings: 1 position · \$300\.00/),
    ).toBeInTheDocument();
    expect(
      within(strip).getByRole("button", { name: "Portfolio" }),
    ).toBeEnabled();
  });
  ...
  it("withdraw candidacy dispatches with electionId and shows reason when unavailable", async () => {
    ...
    const withdraw = within(card).getByRole("button", {
      name: /withdraw candidacy/i,
    });
    expect(withdraw).toBeDisabled();
    expect(within(card).getAllByText(/filing closed/i).length).toBeGreaterThan(
      0,
    );
  ...
  it("candidacy button is disabled while busy", async () => {
    ...
    const card = screen.getByRole("article", { name: "General Election" });
    expect(
      within(card).getByRole("button", { name: /run for office/i }),
    ).toBeDisabled();
  });
  ...
  it("paginates elections 20 per page so every election stays reachable", async () => {
    ...
    expect(
      screen.queryByRole("article", { name: "Race 24" }),
    ).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /next page/i }));
    expect(
      screen.getByRole("article", { name: "Race 24" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("article", { name: "Race 0" }),
    ).not.toBeInTheDocument();
  ...
  it("party cards join and leave via world.actions availability with candidacy warning", async () => {
    ...
    for (const p of world.parties) {
      ...
      expect(
        card.querySelector(`.ahd-mark[data-party-mark="${p.abbreviation}"]`),
      ).not.toBeNull();
    }
  ...
  it("shows the legislature office on Profile and through the Legislature tab", async () => {
    ...
    expect(
      screen.getByRole("article", { name: "Wage Bill" }),
    ).toBeInTheDocument();
  ...
  it("shows no office on Profile without an office", async () => {
    const world = makeWorld({
      legislature: { ...makeWorld().legislature, office: null },
    });
    render(
      <GameScreen
        {...preferencesProps}
  ...
  it("deep-links from Profile finances into Fundraising and preserves the filter on return", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Fundraising actions" }),
    );
    const hub = screen.getByRole("region", { name: "Actions" });
    expect(
      within(hub).getByRole("tab", { name: /fundraising/i }),
    ...
    await navigate(user, "Actions");
    expect(screen.getByRole("tab", { name: /fundraising/i })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
...
describe("GameScreen navigation menu", () => {
  ...
  it("opens a grouped menu with all destinations and closes on selection", async () => {
    ...
    expect(
      within(menu).queryByRole("group", { name: "Character" }),
    ).not.toBeInTheDocument();
    for (const label of [
      "Profile",
      "Actions",
      ...
      expect(
        within(menu).getByRole("group", { name: label }),
      ).toBeInTheDocument();
    }
    const profile = within(menu).getByRole("group", { name: "Profile" });
    expect(
      within(profile).getByRole("button", { name: "Profile" }),
    ).toBeInTheDocument();
    expect(
      within(profile).getByRole("button", { name: "Notifications" }),
    ).toBeInTheDocument();
    expect(
      within(profile).getByRole("button", { name: "Settings" }),
    ).toBeInTheDocument();
    expect(
      within(profile).getByRole("button", { name: "Portfolio" }),
    ).toBeInTheDocument();
    ...
    expect(
      within(actions).getByRole("button", { name: "Actions" }),
    ).toBeInTheDocument();
    const nation = within(menu).getByRole("group", { name: "Nation" });
    await user.click(within(nation).getByRole("button", { name: "Nation" }));
    expect(
      within(nation).getByRole("group", { name: "Politics" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("group", { name: "Government" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("group", { name: "Economy" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("button", { name: "Parties" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("button", { name: "Legislature" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("button", { name: "Elections" }),
    ).toBeInTheDocument();
    expect(
      within(nation).getByRole("button", { name: "National Budget" }),
    ).toBeInTheDocument();
    const worldGroup = within(menu).getByRole("group", { name: "World" });
    await user.click(within(worldGroup).getByRole("button", { name: "World" }));
    expect(
      within(worldGroup).getByRole("group", { name: "Economy" }),
    ).toBeInTheDocument();
    expect(
      within(worldGroup).getByRole("button", { name: "Stock market" }),
    ).toBeInTheDocument();
    expect(
      within(worldGroup).getByRole("button", { name: "Bonds" }),
    ).toBeInTheDocument();
    expect(
      within(worldGroup).getByRole("button", { name: "Banking" }),
    ).toBeInTheDocument();
    expect(
      within(worldGroup).getByRole("button", { name: "News" }),
    ).toBeInTheDocument();
  ...
  it("navigates to a real Profile route with player data", async () => {
    ...
    expect(
      await within(region).findByRole("heading", { name: "Ada" }),
    ).toBeInTheDocument();
  ...
  it("renders Portfolio from world.finance in a region with no tab selected", async () => {
    ...
    within(screen.getByRole("navigation", { name: "Primary" }))
      .getAllByRole("button")
      .forEach((t) => expect(t).not.toHaveAttribute("aria-current", "page"));
  });
  ...
  it("exposes the avatar/profile identity flow and keeps unreachable reference rows absent", async () => {
    ...
    for (const label of ["Profile", "Notifications", "Settings", "Portfolio"]) {
      expect(
        within(identity).getByRole("button", { name: label }),
      ).toBeInTheDocument();
    }
    expect(
      within(menu).getByRole("group", { name: "Actions" }),
    ).toBeInTheDocument();
    ...
    for (const label of [
      ...
      expect(
        within(menu).queryByRole("button", { name: label }),
      ).not.toBeInTheDocument();
    }
  });
  ...
  it("returns to a tab route from a region route", async () => {
    ...
    await user.click(within(menu).getByRole("button", { name: "Portfolio" }));
    expect(
      screen.getByRole("region", { name: "Portfolio" }),
    ).toBeInTheDocument();
    await navigate(user, "Profile");
    expect(screen.getByRole("button", { name: "Profile" })).toHaveAttribute(
      "aria-current",
    ...
    expect(screen.getByRole("main")).toBeInTheDocument();
    expect(
      screen.queryByRole("region", { name: "Portfolio" }),
    ).not.toBeInTheDocument();
  });
});
...
describe("GameScreen search", () => {
  it("restores the last query and results, and keeps the opened result selected across route changes", async () => {
    ...
    const results = {
      ...
      facets: {
        ...
        countries: [
          ...
          { id: "US", label: "United States", count: 1 },
        ],
        regions: [],
      },
    };
    ...
    await user.type(
      screen.getByLabelText("Search your world"),
      "France{Enter}",
    );
    ...
    expect(
      screen.getByRole("button", { name: "United States Nation" }),
    ).toBeInTheDocument();
  });
  ...
  it("opens the exact referendum record a result points at", async () => {
      ...
      referendums: [
        {
          ...
          status: "completed",
          phase: "Completed",
          scope: "Scotland · devolved region",
          yesShare: 55.1,
          finalYesShare: 55.1,
          passed: true,
          turnout: 68.1,
          requestedTurn: 10,
          campaignOpenTurn: 10,
          campaignCloseTurn: 58,
          conversionDeadlineTurn: 70,
          cooldownReadyAtTurn: 130,
          latestPollTurn: 57,
          campaign: {
            active: false,
            yesUnits: 0,
            ...
            groundGame: { presets: [], cohorts: [], available: false },
          },
        },
        {
          id: "referendum-WAL-1",
          ...
          regionId: "WAL",
          regionName: "Wales",
          question: "Should Wales become an independent country?",
          status: "polling",
          phase: "Polling",
          scope: "Wales · devolved region",
          yesShare: 30,
          finalYesShare: null,
          passed: null,
          turnout: null,
    ...
    const searchReferendum = vi.fn(async () => ({
      ...
      results: [
        {
          ...
          id: "referendum-SCO-1",
          title: "Should Scotland become an independent country?",
          description: "Referendum · Scotland",
          countryId: "UK",
          regionId: "SCO",
        },
      ],
      total: 1,
      facets: {
        kinds: [{ id: "referendum", label: "Referendums", count: 1 }],
        countries: [{ id: "UK", label: "United Kingdom", count: 1 }],
        regions: [{ id: "SCO", label: "Scotland", count: 1 }],
      },
    }));
    render(
      <GameScreen
        {...preferencesProps}
        loadProfile={async () => profileFor(world)}
        loadPolitics={loadReferendums}
        search={searchReferendum}
        loadBondMarket={loadBondMarket}
        loadRegions={loadRegions}
        loadCaucusManagement={loadCaucusManagement}
        loadPartyManagement={loadPartyManagement}
    ...
    await user.click(
      await screen.findByRole("button", { name: /Should Scotland/ }),
    );
    expect(
      screen.getByRole("region", { name: "Referendums" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("article", {
        name: "Should Scotland become an independent country?",
      }),
    ).toBeInTheDocument();
  });
});
...
describe("GameScreen status footer", () => {
  it("persists turn, date, player-paced status and five resource buttons", () => {
    ...
    const footer = screen.getByRole("contentinfo", {
      name: "Status and primary navigation",
    });
    expect(within(footer).getByText(/turn 1/i)).toBeInTheDocument();
    expect(
      within(footer).getByText(/January, Week 2, 1952/),
    ).toBeInTheDocument();
    expect(within(footer).getByText(/player paced/i)).toBeInTheDocument();
    expect(
      within(footer).getByRole("button", { name: /action points/i }),
    ).toBeInTheDocument();
    expect(
      within(footer).getByRole("button", { name: /campaign funds/i }),
    ).toBeInTheDocument();
    expect(
      within(footer).getByRole("button", { name: /cash/i }),
    ).toBeInTheDocument();
    expect(
      within(footer).getByRole("button", { name: /influence/i }),
    ).toBeInTheDocument();
    expect(
      within(footer).getByRole("button", { name: /favorability/i }),
    ).toBeInTheDocument();
  });
  ...
  it("leads the footer with the character identity linking to Profile, with the reference-calendar date", async () => {
    ...
    const identity = within(footer).getByRole("button", {
      name: "Profile: Ada",
    });
    expect(identity).toHaveTextContent("Ada");
    expect(
      within(footer).getByText(/Labor · United States/),
    ).toBeInTheDocument();
    ...
    await user.click(
      within(footer).getByRole("button", { name: "Profile: Ada" }),
    );
  ...
  it("captions the resource chips with the reference Profile label ahead of the resource buttons (#223)", () => {
    ...
    const firstResource = within(group).getByRole("button", {
      name: /action points/i,
    });
    expect(
      caption.compareDocumentPosition(firstResource) &
  ...
  it("maps the resumed calendar through the stamped founding offset with no badge (#223)", () => {
    ...
    expect(within(footer).queryByText("Founding")).not.toBeInTheDocument();
    expect(
      within(footer).getByText(/February, Week 1, 1953/),
    ).toBeInTheDocument();
  });
  ...
  it("formats money with finance.currency", () => {
    const world = makeWorld({
      finance: makeFinance({ currency: "EUR", cash: 1200, savings: 300 }),
    });
    render(
      <GameScreen
        {...preferencesProps}
  ...
  it("opens nonmodal cash details with close, real data, and links to Actions, Profile and Portfolio", async () => {
    ...
    expect(
      within(dialog).getByRole("button", { name: /go to actions/i }),
    ).toBeInTheDocument();
    expect(
      within(dialog).getByRole("button", { name: /go to profile/i }),
    ).toBeInTheDocument();
    expect(
      within(dialog).getByRole("button", { name: /go to portfolio/i }),
    ).toBeInTheDocument();
  ...
  it("closes resource details on Escape and returns focus", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: /influence details/i }),
    ).toBeInTheDocument();
    await user.keyboard("{Escape}");
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(
      ...
      ).getByRole("button", { name: /influence/i }),
    ).toHaveFocus();
  });
