...
const DEFAULTS: Preferences = { textSize: "standard", reducedMotion: "system", reducedTransparency: "system", disableAutoplayOnOtherProfiles: false, worldMapSection: "nations" };
...
describe("SettingsPanel", () => {
  ...
  it("reports each user choice through the typed preference callback", async () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith({ ...DEFAULTS, textSize: "large" });
    await user.click(screen.getByRole("radio", { name: "Reduce motion" }));
    expect(onChange).toHaveBeenLastCalledWith({ ...DEFAULTS, reducedMotion: "on" });
  });
  ...
  it("shows a persistence error without hiding the controls", () => {
    render(<SettingsPanel value={{ ...DEFAULTS, textSize: "large", reducedMotion: "off", reducedTransparency: "off" }} onChange={vi.fn()} error="Device preferences could not be saved." />);
