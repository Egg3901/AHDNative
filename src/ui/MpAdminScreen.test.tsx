/**
 * Native admin status screen (#359): read-only maintenance triage gated by
 * server-returned admin permissions. No admin controls mutate anything; no
 * view embeds or links to another client app.
 */
...
describe("MpAdminScreen", () => {
  it("shows the maintenance status to server-confirmed admins", async () => {
    ...
    expect(screen.queryByRole("button", { name: /take (the )?site offline|set maintenance|patch/i })).toBeNull();
  });
