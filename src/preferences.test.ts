...
describe("preferences", () => {
  ...
  it("loads and saves a normalized device preference record", () => {
    ...
    const saved = savePreferences({ textSize: "large", reducedMotion: "on", disableAutoplayOnOtherProfiles: true, worldMapSection: "regions" }, storage);
    expect(saved).toEqual({ value: { textSize: "large", reducedMotion: "on", disableAutoplayOnOtherProfiles: true, worldMapSection: "regions" }, error: null });
  ...
  it("returns defaults and a readable error when device storage fails", () => {
    ...
    expect(savePreferences({ textSize: "large", reducedMotion: "off", disableAutoplayOnOtherProfiles: true, worldMapSection: "nations" }, broken)).toEqual({
      value: { textSize: "large", reducedMotion: "off", disableAutoplayOnOtherProfiles: true, worldMapSection: "nations" },
  ...
  it("applies presentation attributes without requiring storage", () => {
    ...
    applyPreferencesToDocument({ textSize: "large", reducedMotion: "system", disableAutoplayOnOtherProfiles: false, worldMapSection: "nations" }, target);
