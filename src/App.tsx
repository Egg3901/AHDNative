...
export function App() {
  ...
  useEffect(() => {
    ...
    let live = true;
    void Promise.all([worker.choices(), saveRepository.list()]).then(([choices, saved]) => {
      if (live) { setEras(choices); setSaves(saved); }
    }).catch(reason => { if (live) setError(String(reason instanceof Error ? reason.message : reason)); });
    return () => { live = false; worker.dispose(); };
  }, []);
  ...
  async function replaceWorld(operation: (worker: GameClient) => Promise<GameView>, nextSlot: string) {
    ...
    try {
      ...
      setWorld(view); setSlot(nextSlot); setScreen('game');
    } catch (reason) { next.dispose(); throw reason; }
  }
  function start(options: NewGameOptions) {
    void run(() => replaceWorld(worker => worker.create({ ...options, seed: options.seed || crypto.randomUUID() }), crypto.randomUUID()));
  }
  ...
  async function save() {
    if (!client.current || !slot) throw new Error('Start or load a game first.');
    const contents = await client.current.serialize(new Date().toISOString());
    await saveRepository.save(slot, contents);
  ...
  if (screen === 'game' && world) return <GameScreen world={world} busy={busy} error={error} message={message}
    onAdvanceTurn={() => void run(async () => { setWorld(await client.current!.advance()); })}
    onAction={(id, params) => void run(async () => {
      const response = await client.current!.act(id, params); setWorld(response.view);
      if (response.result.ok) setMessage(response.result.message); else setError(response.result.error);
    })}
    onSave={() => void run(save)}
    onExit={() => void run(async () => { await save(); setScreen('home'); })} />;
