...
describe("mpFetch/mpMutate", () => {
  it("resolves bodies and forwards validated args", async () => {
    ...
    expect(
      await mpFetch(unionHost, "union-detail", undefined, undefined, undefined, undefined, "68a000000000000000000001"),
    ).toEqual({
      kind: "ok",
      bodyText: '{"union":{}}',
    });
    expect(unionFetch).toHaveBeenCalledWith(
      "union-detail",
      undefined,
      undefined,
      undefined,
    ...
    const cabinetFetch = vi.fn(async () => '{"position":{}}');
    const cabinetHost = hostWith({ fetch: cabinetFetch });
    expect(
      await mpFetch(cabinetHost, "cabinet-detail", undefined, undefined, undefined, undefined, undefined, "us", "secretary_of_state"),
    ).toEqual({
      kind: "ok",
      bodyText: '{"position":{}}',
    });
    expect(cabinetFetch).toHaveBeenCalledWith(
      "cabinet-detail",
      undefined,
      undefined,
      undefined,
