...
Development preview 0.1.9 is the next source candidate. The latest published source prerelease is `preview/0.1.8-1`; the latest valid internal build is iOS 0.1.7 build 1.11, delivered as a private internal TestFlight build through manual Codemagic to the Owner review group. Windows delivery is a private unsigned x64 owner-review build. These are not public store releases or a 1.0.0 claim; no 0.1.9 build exists yet, and owner feedback and physical-device validation remain open in [issue #124](https://github.com/Egg3901/AHDNative/issues/124). What holds today:
