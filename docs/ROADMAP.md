...
| N09 | Done | Help, settings, search and notifications | Offline guidance and preferences, real search and notifications, clearly marked network resources, and account or feedback routes delegated to the authenticated AHDGame surface |
...
- Help now preserves its offline guide while restoring the reference wiki, about, Discord, supporter, contact and status destinations with visible network-required labels. Account settings and feedback open only inside the AHDGame multiplayer surface, where AHDGame owns authentication and account mutations. Native stores no multiplayer credentials. The Rust command accepts only the named home, settings and feedback routes on the canonical HTTPS origin.
