...
- Catalog classification resolves only the four national metric paths currently backed by Native. Unknown paths are blocked as `unsupportedMetric`, regional effects as `regionalMetrics`, and defense portfolios as `defensePipeline`; no missing metric is created from a default.
