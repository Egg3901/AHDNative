...
Listing commands are live outside the action catalog: `GameSession.listSectorForSale` / `updateSectorListing` / `unlistSectorForSale` run the engine calls on a clone as the player and commit only on ok. Authority is the recorded shareholder roster (player holds >= 1 share). Listings persist as `CorporateSectorAsset.forSale` (null or a positive finite asking price; anything else fails closed at validation and at the save boundary) and project into `sectorAsset.forSale` plus per-sector `forSaleCount`. The company-detail card enables List for sale, Asking price plus Update price, and Unlist only for recorded shareholders. Evidence: `src/game/sectorSaleSession.test.ts` and the #294 block in `src/ui/MarketsPanel.test.tsx`.

## Sector acquisition (#295)

`GameSession.buySectorForSale` runs `buyCorporateSectorForSale` on a clone as the player and commits only on ok: personal cash is debited at the recorded anchor, the seller corporation's `liquidCapital` is credited, the listing clears, and `owner` records `player` while the recorded corporation keeps operating the sector. Buying needs no shareholding; list/update refuse player-owned sectors so a bought sector is never relisted. The company-detail card carries a live Buy (enabled exactly when the command can proceed, each refusal showing its gate reason), an Owner row, and the For Sale section renders one live row per recorded listing. Ownership persists through serialize/load; pre-#295 saves backfill to `corporation`. Evidence: `packages/engine/src/corporation/corporateSectorAcquire.test.ts`, the #295 blocks in `src/game/sectorSaleSession.test.ts` and `src/ui/MarketsPanel.test.tsx`.

Profile corporation card (#51). `ProfileView.corporations` projects the same `MarketListing` objects the company detail renders, filtered to recorded player ownership (`sectorAsset.owner === "player"`). The Profile renders the card between Policy and Finances only when the list is non-empty, with ticker/name, country, sector, market value, share price, player shares, treasury, revenue, and a View company link into the Markets company detail. The reference gates its card on a recorded ceoId and Native records no CEO relationship, so no CEO label is ever shown and share holdings alone never qualify. The engine has no dividend system and player income routing is out of scope, so no salary or dividend values are shown. Offline: pure local projection, no network. Evidence: `src/game/profileCorporation.test.ts` (session, save/reload role-following, shares-are-not-ownership) and `src/ui/ProfileCorporationCard.test.tsx` (rendered omit/link navigation).
