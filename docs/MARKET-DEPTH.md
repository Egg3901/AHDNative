...
## Recorded trade context (#77 slice, 2026-09-18)

Read-only, Markets destination only. `src/game/tradeRoutes.ts` projects one row per listed country (listing count, recorded `economicFactors.tradeGrowth`, recorded FX rate/base/regime) and `MarketListing.orderFlow` carries the recorded order-flow window notionals, multipliers, and `insolventSinceTurn`. `MarketsPanel` renders a Trade routes card plus a per-company Recorded trade context card with explicit empty/unavailable copy. No quotes, spreads, conversions, settlement, bids/asks, issuance, dealer, or restructuring state is shown because the engine records none. Buy/sell behavior and accounting are untouched. Tests: `src/game/tradeRoutes.test.ts` (populated/empty/unavailable, turn/save survival), #77 blocks in `src/ui/MarketsPanel.test.tsx`. Focused runs only: tradeRoutes+markets (43) and MarketsPanel (48) green; full verify/typecheck/build owed to the shared scheduler.

Remaining #77 gaps: FX wallets, quotes, settlement, spreads, order books, history; bilateral/market trade settlement, route restrictions, commodity context UI; corporate issuance and dealer-pool actions; default/restructuring transitions beyond the recorded insolvent marker; atomicity proof across FX trades and defaults.
