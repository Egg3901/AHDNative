...
| Electoral votes from apportioned seats | `packages/engine/src/electionEngine/resolution/apportionment.ts` `electoralVotesFromSeats` | `electoralVotesByState` in `presidentialElectoralCollege.ts` feeds live per-region `houseSeats` through the helper with `eraToPreset(world.meta.era)` + the live year, intersected back to live regions. `presidentialElectoralCollege.test.ts` pins 1953 helper-equivalence (531/266), 2019 AK/HI with no invented DC (535/268), and the post-1961 DC drop. | Yes | Yes | No | Wired for the EV-per-state derivation; outputs are byte-identical to the old direct formula on all shipped packs. DC-as-region and Maine/Nebraska district units still lack Native entities and remain under [#98](https://github.com/Egg3901/AHDNative/issues/98). Do not claim district splits or DC electors. |
...
| Poll projections | `packages/engine/src/actions/polling.ts` `computePollData` and `commissionPoll` | `executeAction` commissions `poll` and `pollLarge` from current home-region demographics, turnout modifiers, party organization and active general-election opponents. It charges the reference costs, persists `lastPoll` or `lastPollLarge`, and projects both through `GameView.polls` into the Actions page. | Yes | Yes | Yes | Public engine tests cover same-turn inputs, atomic rejection, quick/full payloads and save round-trip. The integrated polling journey covers commissioning and reading the stored result. [#38](https://github.com/Egg3901/AHDNative/issues/38). |
| Subsidy-rate player lever | `packages/engine/src/budget/subsidyBudget.ts` `enactNationalSubsidy`/`endNationalSubsidy` (composite-key upsert, fixed reference bonus). | `executeAction` `setSubsidyRate` (HoS-only, national scope) writes `world.subsidies`; `subsidyBudgetPhase` charges the line next turn. | Partial | No | No | Enact/end lifecycle with sector targeting is live; state scope, strategy targeting, and the HoS-hub surfacing row remain under [#94](https://github.com/Egg3901/AHDNative/issues/94). |
