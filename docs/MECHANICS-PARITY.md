...
3) Electoral tie-break and scope honesty - DONE (tie part, 2026-09-18); VP/governor effects still marked, district gap disclosed
- Change landed: `presidentialElectoralCollege.ts` exact-tie rule corrected from alphabetical to first-seen candidate order, matching mainline `src/lib/elections/electoralVoteService.ts:83-88` (stable votes-descending sort, no secondary key). Source check proved the earlier `sha256` expectation wrong for this path: mainline `sha256` tiebreaks are contingent-ballot/deadlock only (`contingentElection.ts:154,247,283`). Still deferred: wiring `vpHomeState *1.03` and governor endorsement `*1.015` from `src/lib/presidentialElectionEngine.ts:945-955` into the per-state tally (needs candidate home-state/governor entities Native lacks); ME/NE district gap (no district entities; `tallyAdapter.ts` single contest per state). No physical-device claims; Linux vitest only.
- Seam: `createWorld({seed:"ec-test",playerName:"Tester",countryId:"US",era:"1953"})`, hand-built `stateTallyStates` with an exact tie (`WY: {Zed:100, Alpha:100}` and the flipped order), plus a JSON save/reload round trip, then `allocateElectoralVotes`.
- Verified result: `{Zed:100, Alpha:100}` gives `stateWinners[WY]="Zed"`; flipped gives `"Alpha"`; JSON round trip preserves the winner with `evByCandidate={Zed:3}`, `totalEv=3`. `electoralVotesByState` at 1953 still totals 531 / majority 266 (matching `apportionment.ts` `HOUSE_SEATS_1953`). If VP/governor wiring lands, a tally where `vpHomeStateByCandidate.get(cid)=="CA"` must show `votes *1.03` and a governor-endorsed candidate `*1.015`, matching `presidentialElectionEngine.ts:945-955`.
...
The inventory above describes the imported baseline. [Referendum variance](REFERENDUM-PARITY.md) now follows the AHDGame per-record FNV hash and consumes no shared RNG. This closes only the variance-source difference, not the lifecycle. [TFP](GROWTH-PARITY.md) now uses the reference basket with exact prior-turn national metric keys when present; missing inputs retain reference defaults. Default worlds still lack the full state-metric inputs and phase-order parity remains open. Neither M02 nor M04 is complete. Presidential per-state canvass/GOTV turnout wiring (#141): `packages/engine/src/elections/tallyAdapter.ts` `realAccumulatePresident` now threads the race id into every per-state slice (and the nationwide fallback), so active-campaign `canvassModifiers` shape the per-group live turnouts exactly as the down-ballot path already does. Reference: `presidentialElectionEngine.ts` resolves per-state turnout "with GOTV/canvassing/suppression modifiers applied". No tally math changed. Evidence: `packages/engine/src/elections/presidentialCanvassTurnout.test.ts` (fails with the threading reverted, passes with it; save/reload determinism pinned). The live tally stays reachable through the unchanged `projectPolitics` counted-vote/primary/presidential views and the existing Liquid Glass `PoliticsPanel`; no UI change was needed. Remaining limits: VP home-state and governor endorsement multipliers still unported, exact-tie behavior still alphabetical (see row above), the nationwide fallback path is defensive-only on shipped packs, and validation ran on the shared Linux host only with no device (390px/Dynamic Island, desktop) re-measurement and no server differential run. The `debatePrep` character action (#37) is ported at coded values (1 AP, 15% roll, Debate +1 capped at 10) through the catalog, `executeAction`, saveable `player.stats.debate`, and the Intelligence hub; see [engine adaptations](ENGINE-ADAPTATIONS.md). Debate decay and participation practice rolls remain unported.
